    ...
    internal static class TabCrowding
    {
        ...
        private static void Set(GameObject tab, TextMeshProUGUI label, bool compact)
        {
            ...
            if (compact && !_announced)
            {
                ...
                // whoever is looking at the bar wondering where ROULETTE went.
                RouletteClientPlugin.Log.LogInfo(
                    "[Roulette] the task bar is crowded, so the tab is showing its pip without a label. "
        ...
        private static void Census(List<TextMeshProUGUI> labels)
        {
            ...
            RouletteClientPlugin.Log.LogInfo(
                $"[Roulette] task bar: {labels.Count} labels, {wrapped} single word(s) broken across lines.");
