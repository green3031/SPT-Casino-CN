...
///
/// Deliberately not stored in the SPT profile -- adding fields there would change
/// the profile schema, which is what makes some mods require a wipe when they are
/// removed. Same reasoning, and the same shape, as Blackjack's store of the same
/// name; not shared with it because each table resolves its own <c>IStatsStore</c>
/// from its own DI container, the same way each table has its own Bank and Escrow.
/// </summary>
[Injectable(InjectionType.Singleton)]
public class StatsStore : IStatsStore
{
    // Named per table, like escrow-slots.json: all four tables share one mod
    // folder now, and a bare stats.json would be Blackjack and this machine
    // overwriting each other's record.
    private const string FileName = "stats-slots.json";
    ...
    // Writes are serialised: a pull settling on one thread while another reads
        ...
        ModHelper modHelper)
    {
        ...
        {
            Writable = false;
            _logger.Error($"Slots: stats folder is not writable at {_path} -- {ex.Message}");
        }
    }
    ...
    public void Save(MongoId sessionId, PlayerStats stats)
    {
        ...
        lock (_writeLock)
        {
            ...
            {
                // Losing stats is annoying; losing the pull because stats failed to
                // write would be worse. Log and carry on.
                _logger.Error($"Slots: could not write stats to {_path} -- {ex.Message}");
            }
        }
    }
    ...
    private ConcurrentDictionary<string, PlayerStats> Load()
    {
        ...
        catch (Exception ex)
        {
            ...
            // old file is overwritten on the next settled pull.
            _logger.Error($"Slots: stats file at {_path} is unreadable, starting fresh -- {ex.Message}");
            return new ConcurrentDictionary<string, PlayerStats>();
        }
    }
}
