...
///
/// Ported from Blackjack, where it was written because
/// <see cref="JsonStringEnumConverter"/> only claims scalar enum types -- a property
/// holding a collection of them needs its own converter. Without this,
/// AvailableActions goes over the wire as [1, 0, 2] and the client has to know that
/// Play is 1, an ordering it should never depend on and one that changes silently
/// the moment somebody inserts a value into the middle of the enum.
///
/// The read side accepts either form, because a name is what we emit but a
/// hand-written request may still send numbers.
/// </summary>
