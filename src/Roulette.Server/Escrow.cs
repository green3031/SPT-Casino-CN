...
{
    private const string FileName = "escrow-roulette.json";
        ...
        ModHelper modHelper)
    {
        ...
        if (_held.IsEmpty)
        {
            ...
            if (carried is not null)
            {
                ...
                {
                    _logger.Info(
                        $"[Roulette] carried {_held.Count} unfinished session(s) over from {carried}.");
                    Flush();
                }
            }
        }
    ...
    private ConcurrentDictionary<string, OutstandingStake> Load(string? from = null)
    {
        ...
        {
            _logger.Error($"[Roulette] escrow file at {source} is unreadable -- {ex.Message}");
            return new ConcurrentDictionary<string, OutstandingStake>();
        }
    }
}
