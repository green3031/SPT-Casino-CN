...
namespace Casino.Client
{
    ...
    internal static class TabCrowding
    {
        ...
        private static void Set(GameObject tab, TextMeshProUGUI label, bool compact)
        {
            ...
            if (compact && !_announced)
            {
                ...
                CasinoPlugin.Log.LogInfo(
                    "[Casino] the task bar is crowded, so the tab is showing its pip without a label. "
        ...
        private static void Census(List<TextMeshProUGUI> labels)
        {
            ...
            CasinoPlugin.Log.LogInfo(
                $"[Casino] task bar: {labels.Count} labels, {wrapped} single word(s) broken across lines.");
