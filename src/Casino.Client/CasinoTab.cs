...
namespace Casino.Client
{
    ...
    /// </summary>
    internal static class CasinoTab
    {
        /// <summary>
        ...
        /// </summary>
        private const string TabName = "CasinoTab";
        ...
        private static bool OnRight =>
            CasinoPlugin.TabOnRight != null && CasinoPlugin.TabOnRight.Value;

        private static bool Wanted =>
            CasinoPlugin.ShowTaskBarTab == null || CasinoPlugin.ShowTaskBarTab.Value;
        ...
        internal static IEnumerator Heartbeat()
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    CasinoPlugin.Log.LogError("[Casino] task-bar tab: " + ex);
                }
            }
        }
        ...
        private static Dictionary<EMenuType, AnimatedToggle> Keyed(MenuTaskBar bar)
        {
            ...
            CasinoPlugin.Log.LogWarning(
                "[Casino] MenuTaskBar._toggleButtons could not be read; falling back to its children.");
        ...
        private static void Install(MenuTaskBar bar)
        {
            ...
            CasinoPlugin.Log.LogInfo(
                $"[Casino] task-bar tab added, cloned from '{from.name}' " +
        ...
        private static void Describe(Dictionary<EMenuType, AnimatedToggle> tabs, List<AnimatedToggle> row)
        {
            ...
            CasinoPlugin.Log.LogInfo("[Casino] task bar: " + string.Join(", ", names.ToArray()));
        }
        ...
        private static void Neuter(GameObject clone)
        {
            ...
            foreach (var component in clone.GetComponentsInChildren<MonoBehaviour>(true))
            {
                ...
                {
                    tooltip.SetMessageText("轮盘", true);
                    continue;
                }
            ...
            if (!_describedComponents && stopped.Count > 0)
            {
                ...
                CasinoPlugin.Log.LogInfo(
                    "[Casino] switched off on the cloned tab: " +
        ...
        {
            if (_measured || CasinoPlugin.Instance == null)
            {
                return;
            }

            _measured = true;
            CasinoPlugin.Instance.StartCoroutine(Report(template, clone));
        }
        ...
        private static IEnumerator Report(Transform template, Transform clone)
        {
            ...
            try
            {
                CasinoPlugin.Log.LogInfo("[Casino] tab, as laid out --");
                CasinoPlugin.Log.LogInfo("  template " + Sizes(template));
                CasinoPlugin.Log.LogInfo("  ours     " + Sizes(clone));
            }
            catch (Exception ex)
            {
                CasinoPlugin.Log.LogWarning("[Casino] could not measure the tab: " + ex.Message);
            }
        }
    ...
    internal sealed class RouletteTabClick : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        private void Update()
        {
            ...
            if (live != _wasLive)
            {
                ...
                CasinoPlugin.Log.LogInfo(live
                    ? "[Casino] tab live again."
                    : "[Casino] tab dim -- InRaid=" + CasinoTab.InRaid
                        + " (" + CasinoTab.DimReason() + "), mirrorGroup="
        ...
        ///
        /// <see cref="CasinoTab.InRaid"/> is first and stands on its own, because it is
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            CasinoPlugin.Log.LogInfo("[Casino] task-bar tab clicked");
            CasinoLobby.Toggle();
        }
